...
CUSTOMER = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/3GB/customer' USING PigStorage(',') AS (C_CUSTKEY: int, C_NAME: chararray, C_ADDRESS: chararray, C_NATIONKEY: int, C_PHONE: chararray, C_ACCTBAL: int, C_MKTSEGMENT: chararray, C_COMMENT: chararray);
ORDERS = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/3GB/orders' USING PigStorage(',') AS (O_ORDERKEY: int, O_CUSTKEY: int, O_ORDERSTATUS: int, O_TOTALPRICE: int, O_ORDERDATE: long, O_ORDERPRIORITY: int, O_CLERK: chararray, O_SHIPPRIORITY: int, O_COMMENT: chararray);
LINEITEM = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/3GB/lineitem' USING PigStorage(',') AS (L_ORDERKEY: int, L_LINENUMBER: int, L_PARTKEY: int, L_SUPPKEY: int, L_QUANTITY: int, L_EXTENDEDPRICE: int, L_DISCOUNT: int, L_TAX: int, L_RETURNFLAG: int, L_LINESTATUS: chararray, L_SHIPDATE: long, L_COMMITDATE: long, L_RECEIPTDATE: long, L_SHIPINSTRUCT: chararray, L_SHIPMODE: chararray, L_COMMENT: chararray);
SUPPLIER = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/3GB/supplier' USING PigStorage(',') AS (S_SUPPKEY: int, S_NAME: chararray, S_ADDRESS: chararray, S_NATIONKEY: int, S_PHONE: chararray, S_ACCTBAL: int, S_COMMENT: chararray);
NATION = LOAD'hdfs://167.205.35.25:9000/user/willyTA/3GB/nation' USING PigStorage(',') AS (N_NATIONKEY:int, N_NAME: chararray, N_REGIONKEY: int, N_COMMENT: chararray);
REGION = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/3GB/region' USING PigStorage(',') AS (R_REGIONKEY: int, R_NAME: chararray, R_COMMENT: chararray);

CUSTOMER = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/testing/customer' USING PigStorage(',') AS (C_CUSTKEY: int, C_NAME: chararray, C_ADDRESS: chararray, C_NATIONKEY: int, C_PHONE: chararray, C_ACCTBAL: int, C_MKTSEGMENT: chararray, C_COMMENT: chararray);
ORDERS = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/testing/orders' USING PigStorage(',') AS (O_ORDERKEY: int, O_CUSTKEY: int, O_ORDERSTATUS: int, O_TOTALPRICE: int, O_ORDERDATE: long, O_ORDERPRIORITY: int, O_CLERK: chararray, O_SHIPPRIORITY: int, O_COMMENT: chararray);
LINEITEM = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/testing/lineitem' USING PigStorage(',') AS (L_ORDERKEY: int, L_LINENUMBER: int, L_PARTKEY: int, L_SUPPKEY: int, L_QUANTITY: int, L_EXTENDEDPRICE: int, L_DISCOUNT: int, L_TAX: int, L_RETURNFLAG: int, L_LINESTATUS: chararray, L_SHIPDATE: long, L_COMMITDATE: long, L_RECEIPTDATE: long, L_SHIPINSTRUCT: chararray, L_SHIPMODE: chararray, L_COMMENT: chararray);
SUPPLIER = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/testing/supplier' USING PigStorage(',') AS (S_SUPPKEY: int, S_NAME: chararray, S_ADDRESS: chararray, S_NATIONKEY: int, S_PHONE: chararray, S_ACCTBAL: int, S_COMMENT: chararray);
NATION = LOAD'hdfs://167.205.35.25:9000/user/willyTA/testing/nation' USING PigStorage(',') AS (N_NATIONKEY:int, N_NAME: chararray, N_REGIONKEY: int, N_COMMENT: chararray);
REGION = LOAD 'hdfs://167.205.35.25:9000/user/willyTA/testing/region' USING PigStorage(',') AS (R_REGIONKEY: int, R_NAME: chararray, R_COMMENT: chararray);
...
tmp7 = FILTER tmp6 BY ORDERS::O_ORDERDATE>=1338508800000L AND ORDERS::O_ORDERDATE<=1346457600000L AND REGION::R_NAME=='224GFtqxqJ';
...
tmp9 = FOREACH tmp8{
	x = FOREACH tmp7 GENERATE (double) (LINEITEM::L_EXTENDEDPRICE * (100 - LINEITEM::L_DISCOUNT))/(double)100 as calc;
...
STORE tmp9 INTO 'hdfs://167.205.35.25:9000/user/willyTA/3GB/result-query5' USING PigStorage(',');
STORE tmp9 INTO 'hdfs://167.205.35.25:9000/user/willyTA/testing/result-query5' USING PigStorage(',');
